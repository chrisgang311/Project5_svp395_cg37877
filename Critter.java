...
/**
 * The Critter class is responsible for all of the actions taken by the
 * critters. The critters are created here and added to the population of
 * critters which is stored as a List. Critters can also be made using this
 * class as a parent class. Furthermore, critters are given their colors and
...
public abstract class Critter {
	...
	protected String look(int direction, boolean steps) {
		boolean xEdge = false;
		boolean yEdge = false;
		int stepSize = steps ? 2 : 1;
		int xCheck = x_coord;
		int yCheck = y_coord;

		// Find exceptions for critters at edges of map
		// x_coord is at left edge
		if (xCheck - stepSize < 0 && direction >= 3 && direction <= 5) {
			xCheck = Params.world_width + xCheck - stepSize;
			xEdge = true;
		}
		// y_coord is at top edge
		if (yCheck - stepSize < 0 && direction >= 1 && direction <= 3) {
			yCheck = Params.world_height + yCheck - stepSize;
			yEdge = true;
		}
		// x_coord is at right edge
		if (xCheck + stepSize > Params.world_width - 1 && (direction == 1 || direction == 0 || direction == 7)) {
			xCheck = (x_coord + stepSize) % Params.world_width;
			xEdge = true;
		}
		// y_coord is at bottom edge
		if (yCheck + stepSize > Params.world_height - 1 && direction >= 5 && direction <= 7) {
			yCheck = (y_coord + stepSize) % Params.world_height;
		...
		for (Critter c : population) {
			if (c.x_coord == xCheck && c.y_coord == yCheck) {
				return c.toString();
			}
		}
		return null;
	...
	/**
	 * The move method takes in a stepSize and direction and moves the critter
	 * in that direction with the given step size. If the critter goes over the
	 * edge of the map it appears at the opposite edge.
	 * 
	 * @param stepSize
	 *            size of step critter will take
	 * @param direction
	 *            direction critter will move in
	 */
	...
	/**
	 * Reproduce will initialize the critter offspring passed in the parameter.
	 * The offspring will be placed in an adjacent space of the parent
	 * 
	 * @param offspring
	 *            the critter object that will be initialized
	 * @param direction
	 *            the direction the critter offspring will be placed
	 */
	...
	public static void makeCritter(String critter_class_name) throws InvalidCritterException {
		try {
			Class critterSub = Class.forName("assignment5." + critter_class_name);
	...
	public static List<Critter> getInstances(String critter_class_name) throws InvalidCritterException {
		...
		try {
			for (Critter c : population) {
				if (Class.forName("assignment5." + critter_class_name).isInstance(c)) {
					result.add(c);
				}
			}
		} catch (ClassNotFoundException e) {
	...
	/**
	 * This method will move each critter using its doTimeStep method and will
	 * resolve conflicts between critters. It will also remove critters if they
	 * are dead. Finally the method will add offspring and algae to the critter
	...
	public static void worldTimeStep() {
		...
		// If a place in the map is greater than 1 then 2 or more critter occupy
		// 1 place
	...
	/**
	 * Resolves conflicts between critters and returns an ArrayList of critters
	 * specifying the critters that have moved during this method.
	 * 
	 * @param fightList
	 *            the list of critters that are in conflict
		...
		while (fightList.size() > 1) {
			...
			if (fightList.size() >= 2) {
				if (fightList.get(0).x_coord == fightList.get(1).x_coord
						&& fightList.get(0).y_coord == fightList.get(1).y_coord && fightList.get(0).energy > 0
	...
	/**
	 * Draws critters directly to the GUI.
	 * 
	 * @param shape
	 *            the shape of the critter
	 * @param outline
	 *            the outline color of the critter
	 * @param fill
	 *            the fill color of the critter
	 * @param x_coord
	 *            the x coordinate where the critter will be placed
	 * @param y_coord
	 *            the y coordinate where the critter will be placed
	 */
	static void drawCritter(CritterShape shape, Color outline, Color fill, int x_coord, int y_coord) {
		Main.context.setFill(fill);
		Main.context.setStroke(outline);

		switch (shape) {
		case CIRCLE:
			Main.context.fillOval(x_coord * Main.scale, y_coord * Main.scale, 4, 4);
			Main.context.strokeOval(x_coord * Main.scale, y_coord * Main.scale, 4, 4);
			break;
		case SQUARE:
			Main.context.fillRect(x_coord * Main.scale, y_coord * Main.scale, 4, 4);
			Main.context.strokeRect(x_coord * Main.scale, y_coord * Main.scale, 4, 4);
			break;
		case TRIANGLE:
			Main.context.fillRect(x_coord * Main.scale, y_coord * Main.scale, 4, 4);
			Main.context.strokeRect(x_coord * Main.scale, y_coord * Main.scale, 4, 4);
	...
	/**
	 * Displays the critter population on the console in their respective
	 * coordinates.
	 */
	public static void displayWorld() {
		...
		for (Critter c : population) {
			shape = c.viewShape();
			outline = c.viewOutlineColor();
			fill = c.viewFillColor();
			drawCritter(shape, outline, fill, c.x_coord, c.y_coord);
		}
		Main.displayStats();
